...
int main(int argc, char* argv[]) {
	...
	if (line->HasSwitch("h") || line->HasSwitch("help") || mpd_url.empty()) {
		const char* help_str =
				"Usage: quic_client [options] <mpd_url>\n"
						"\n"
	...
	VLOG(1) << "server host: " << FLAGS_host << " port: " << FLAGS_port
			...
			<< " quiet: " << FLAGS_quiet << " quic-version: "
			<< FLAGS_quic_version << " version_mismatch_ok: "
			<< FLAGS_version_mismatch_ok << " redirect_is_success: "
			<< FLAGS_redirect_is_success << " initial_mtu: "
			<< FLAGS_initial_mtu << " folder:" << FLAGS_folder;
	...
	if (line->HasSwitch("disable-certificate-verification")) {
		...
		proof_verifier.reset(
				new ProofVerifierChromium(cert_verifier.get(),
						ct_policy_enforcer.get(),
						transport_security_state.get(), ct_verifier.get()));
	}
	net::QuicSimpleClient client(net::QuicSocketAddress(ip_addr, port),
	...
	if (!client.Connect()) {
		...
		if (FLAGS_version_mismatch_ok && error == net::QUIC_INVALID_VERSION) {
			cout << "Server talks QUIC, but none of the versions supported by "
					<< "this client: " << QuicVersionVectorToString(versions)
					<< endl;
	...
	while (true) {
		...
		if (!FLAGS_quiet) {
			...
			if (!FLAGS_body_hex.empty()) {
				...
				cout << "body:\n"
						<< QuicTextUtils::HexDump(
								QuicTextUtils::HexDecode(FLAGS_body_hex))
						<< endl;
			} else {
				//cout << "body: " << body << endl;
			}
			...
			if (!FLAGS_body_hex.empty()) {
				...
				cout << "body:\n" << QuicTextUtils::HexDump(response_body)
						<< endl;
			} else {
				...
				outf << response_body;
				cout << "file_size:" << response_body.size() << endl;
		...
		if (response_code >= 200 && response_code < 300) {
			...
			if (FLAGS_redirect_is_success) {
				cout << "Request succeeded (redirect " << response_code << ")."
						<< endl;
				//	return 0;
			} else {
				cout << "Request failed (redirect " << response_code << ")."
						<< endl;
				return 1;
			}
		} else {
